...
| `~/.openviking/codex-plugin-state/<session_id>.json` | Per-session state: `ovSessionId` (resolved ID, null once committed), `lastCommittedOvSessionId` (resume archive identity), `transcriptPath` (last rollout seen, used by the SessionStart sweep to catch up unsent turns), `capturedTurnCount`, `lastUpdatedAt`. |
