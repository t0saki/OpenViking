...
test("shared ovcli recallCompress=auto enables Codex local compression", async () => {
  const dir = await mkdtemp(join(tmpdir(), "ov-codex-config-"));
  const cliConfig = join(dir, "ovcli.conf");
  const ovConfig = join(dir, "ov.conf");
  const keys = [
    "OPENVIKING_CLI_CONFIG_FILE",
    "OPENVIKING_CONFIG_FILE",
    "OPENVIKING_RECALL_COMPRESS",
  ...
  try {
    ...
    await writeFile(ovConfig, JSON.stringify({
      codex: {
        recallCompress: false,
      },
    }));
    process.env.OPENVIKING_CLI_CONFIG_FILE = cliConfig;
    process.env.OPENVIKING_CONFIG_FILE = ovConfig;
    delete process.env.OPENVIKING_RECALL_COMPRESS;
    assert.equal(loadConfig().recallCompress, true);
  } finally {
    for (const key of keys) {
      if (previous[key] === undefined) delete process.env[key];
      else process.env[key] = previous[key];
