...
test("catalog lists openviking and its source points at this plugin dir", () => {
  ...
  assert.ok(rel.endsWith("examples/codex-plugin"), `source path must point at examples/codex-plugin, got "${rel}"`);
...
test("required plugin files are present", () => {
  for (const rel of [
    ...
    "skills/ov-plugin-doctor/SKILL.md",
    "skills/ov-plugin-doctor/reference.md",
    "scripts/ov-plugin-doctor.mjs",
