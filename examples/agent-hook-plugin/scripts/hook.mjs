...
import { deriveHarnessSessionId, formatReadableSessionId, uuidV7TimeMs } from "../../memory-plugin-shared/lib/session-model.mjs";
...
// Called under the existing per-native-session lock, before any early return
// or network work. Persist independently of what a host's capture returns.
async function readSessionState(ctx) {
  const state = await readHookState(clientId, ctx.nativeSessionId);
  if (!state.ovSessionId) {
    const legacy = deriveHarnessSessionId(host.prefix, ctx.nativeSessionId);
    const hasActivity = Object.keys(state).some((key) => key !== "version");
    state.ovSessionId = hasActivity || ctx.nativeSessionId.startsWith("cwd-")
      ? legacy
      : formatReadableSessionId(host.harness, uuidV7TimeMs(ctx.nativeSessionId) ?? Date.now(), ctx.nativeSessionId) || legacy;
    await writeHookState(clientId, ctx.nativeSessionId, state);
  }
  ctx.sessionId = state.ovSessionId;
  return state;
}
