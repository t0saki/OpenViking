 ...
 *
 * Usage:
 *   node ov-plugin-doctor.mjs [<client>] [--json] [--offline] [--timeout <ms>] [--no-color]
 *
...
import {
  ...
  sweepEnv,
  tryJson,
} from "../../plugin-shared/lib/doctor-core.mjs";
...
function checkInstall(report) {
  ...
  if (missing.length) report.fail("plugin files missing", missing.join(", "), `bash <(curl -fsSL https://raw.githubusercontent.com/volcengine/OpenViking/main/examples/plugin-shared/install.sh) --harness ${CLIENT}`);
  ...
  const shared = resolvePath(PLUGIN_ROOT, "..", "plugin-shared", "lib");
