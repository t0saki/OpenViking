...
export function buildKimicodeCapturePlan(turns, state = {}, cfg = {}) {
  ...
  for (const turn of turns) {
    ...
    candidates.push({ dedupKey: kimicodeTurnDedupKey(turn), turn, content: decision.text });
  }
...
export function applyKimicodeCaptureResult(state, plan, result) {
  ...
  const pendingPrompt = cleanKimicodeText(state.pendingPrompt?.prompt || "");
  let pendingPromptItem = null;
  if (pendingPrompt) {
    for (let index = plan.candidates.length - 1; index >= 0; index--) {
      const item = plan.candidates[index];
      if (item.turn.role === "user" && cleanKimicodeText(item.turn.content) === pendingPrompt) {
