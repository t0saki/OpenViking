...
- **Stop** — captures the finished turn and commits the OpenViking session. Cursor also runs this before a compaction and at session end, and so does Kimi Code, which additionally captures on an interrupt; ZCode answers first and finishes the writes in a detached worker.
...
`hosts/<host>/` holds only what a host reads as configuration — `hooks.json`, `.mcp.json`, `openviking.integration.json`, plus Cursor's rule and skill and Kimi Code's `kimi.plugin.json`. Everything executable stays one level up, because `../../memory-plugin-shared/lib` is the path that resolves both in this repository and in an installed `~/.openviking/agent-integrations/<client>/`.
...
- **Cursor** — seven events, including the `preCompact` and `sessionEnd` only Kimi Code otherwise has. Commits on Stop once `capturedSinceCommit` reaches the threshold, and unconditionally before a compaction. Sessions are `cu-`. See the [Cursor guide](../../docs/en/agent-integrations/12-cursor.md).
...
- **Kimi Code** — seven events, the only host here with `Interrupt` alongside `PreCompact` and `SessionEnd`. Hooks live in `~/.kimi-code/config.toml` as a `[[hooks]]` array and injection is plain stdout text, so the profile rides the first prompt rather than the session start, which only observes. `agents/main/wire.jsonl`, found through `session_index.jsonl`, is the incremental transcript. `Interrupt` fires instead of `Stop` and captures inline instead of detaching. Sessions are `kc-`. [DESIGN.md](./DESIGN.md) records the verified extension surface.
...
The client defaults to the one this copy was installed for; pass `cursor`, `trae`, `trae-cn`, `zcode` or `kimicode` as an argument to override it, drop `--offline` to probe the server as well, and add `--json` for a machine-readable report.
