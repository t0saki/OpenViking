...
Cursor、TRAE、TRAE CN、ZCode 与 Kimi Code 的安装方式相同：共享安装脚本把生命周期 hook 与 MCP server 条目写进宿主自己的配置文件，并在集成目录旁装配 OpenViking 运行时。五家都不需要注册 marketplace，也不需要单独配置 MCP。
...
- **Stop** — 捕获完成的回合并提交 OpenViking 会话。Cursor 另外在压缩前与会话结束时执行，Kimi Code 也是，并额外在中断时采集；ZCode 先应答，再在 detached worker 里完成写入。
...
`hosts/<host>/` 只放宿主当作配置读取的东西——`hooks.json`、`.mcp.json`、`openviking.integration.json`，以及 Cursor 的 rule 与 skill、Kimi Code 的 `kimi.plugin.json`。可执行文件一律放在上一层，因为 `../../memory-plugin-shared/lib` 这条相对路径要在本仓库和安装后的 `~/.openviking/agent-integrations/<client>/` 两处同时成立。
...
- **Cursor** — 七个事件，其中 `preCompact` 与 `sessionEnd` 在本插件里只有 Kimi Code 也有。Stop 时 `capturedSinceCommit` 达到阈值才 commit，压缩前无条件 commit。会话前缀 `cu-`。见 [Cursor 接入文档](../../docs/zh/agent-integrations/12-cursor.md)。
...
- **Kimi Code** — 七个事件，是这里唯一同时有 `Interrupt`、`PreCompact` 与 `SessionEnd` 的宿主。Hook 写在 `~/.kimi-code/config.toml` 的 `[[hooks]]` 数组里，注入是纯文本 stdout，因此 profile 随首条 prompt 注入而不是会话开始——后者只观察。增量对话源是 `session_index.jsonl` 指向的 `agents/main/wire.jsonl`。`Interrupt` 代替 `Stop` 触发，同步采集而不 detach。会话前缀 `kc-`。已验证的扩展面记录在 [DESIGN.md](./DESIGN.md)。
...
client 默认取这份副本安装时对应的那个；传 `cursor`、`trae`、`trae-cn`、`zcode` 或 `kimicode` 可以覆盖，去掉 `--offline` 会连带探测服务端，加 `--json` 输出机器可读的报告。
