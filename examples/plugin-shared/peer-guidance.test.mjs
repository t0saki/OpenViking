...
const REFERENCES = [
  "examples/claude-code-plugin/skills/ov-plugin-doctor/reference.md",
  "examples/codex-plugin/skills/ov-plugin-doctor/reference.md",
];

test("every surface that teaches the peer file quotes the same snippet", () => {
  const carriers = [
    "examples/skills/openviking/SKILL.md",
    ...REFERENCES,
...
test("no document still promises the working directory, or a command that does not exist", () => {
  const files = [
    ...
    ...walk(join(ROOT, "examples", "plugin-shared", "lib"), (p) => p.endsWith(".mjs")),
  ];
