...
const KNOB_READERS = [
  "examples/plugin-shared/lib/plugin-config.mjs",
  "examples/claude-code-plugin/scripts/config.mjs",
  "examples/codex-plugin/scripts/config.mjs",
  "examples/opencode-plugin/lib/config.mjs",
  "examples/dsh-plugin/config.mjs",
  "examples/pi-coding-agent-extension/config.ts",
  "examples/plugin-shared/lib/agent-hook-runtime.mjs",
];
...
test("exactly the knobs recall-core gates are marked send-only-when-configured", async () => {
  ...
  const source = await readFile(join(ROOT, "examples/plugin-shared/lib/recall-core.mjs"), "utf-8");
  const gated = new Set();
