...
#   bash <(curl -fsSL https://ovrelease.tos-cn-beijing.volces.com/plugin-shared/install.sh) --dist tos
# Non-interactive:
...
install_dsh() {
  ...
  if [ "$SOURCE_MODE" = "dev" ] && local_dir="$(plugin_dir_on_disk dsh-plugin)"; then
    local packed
...
dsh_pack_local() { # dsh_pack_local <plugin-dir> -> tarball path
  local dir="$1" dest="$OV_HOME/dsh-plugin" name fingerprint target
  ...
  target="$dest/$fingerprint/openviking-dsh-plugin.tgz"
  if [ -f "$target" ]; then
    printf '%s' "$target"
    return 0
  fi
...
resolve_self_checkout() {
  ...
  if [ -e "$dir/../../.git" ] && [ -d "$dir/../claude-code-plugin" ]; then
...
require_install_lib_dir() {
  ...
  err "$(t 'Installer runtime not found:' '未找到安装器运行时：') plugin-shared/lib/install"
  return 1
}
...
install_claude_legacy() {
  ...
  plugin_dir="$(plugin_dir_on_disk claude-code-plugin)" || {
  ...
  case "$SOURCE_MODE" in
    remote)
      ...
      codex_cmd plugin marketplace add "$REPO_URL" --ref "$REPO_REF" \
        --sparse examples/codex-plugin --sparse .agents >/dev/null 2>&1 || \
...
assemble_agent_integration() { # assemble_agent_integration <host> <dest-name>
  ...
  shared="$(plugin_dir_on_disk plugin-shared)" || {
    err "$(t 'Shared agent runtime not found.' '未找到共享 Agent 运行时。')"
    return 1
  }
  shared_dest="$OV_HOME/agent-integrations/plugin-shared/lib"
...
install_cursor() {
  ...
  cp "$root/hosts/cursor/rules/openviking.mdc" "$HOME/.cursor/rules/openviking.mdc"
  skill_tmp="$HOME/.cursor/skills/openviking.tmp"
  rm -rf "$skill_tmp"
...
sync_shared_runtime() {
  local shared root
  shared="$(plugin_dir_on_disk plugin-shared)" || return 0
  shared="$(cd "$shared" && pwd -P)" || return 1
  root="$(cd "$shared/../.." 2>/dev/null && pwd)" || return 0
  [ -f "$shared/sync.mjs" ] && [ -d "$root/examples/plugin-shared" ] || return 0
...
install_pi() {
  ...
  if [ ! -f "$plugin_dir/shared/credentials.mjs" ]; then
    warn "$(t 'pi extension shared runtime is missing; run node examples/plugin-shared/sync.mjs and retry.' '未找到 pi 扩展的共享运行时；请先运行 node examples/plugin-shared/sync.mjs 再重试。')"
    return 0
  fi
...
EOF
  ...
  if contains_harness cursor; then
    if grep -q 'scripts/hook.mjs' "$HOME/.cursor/hooks.json" 2>/dev/null \
      ...
      && [ -f "$OV_HOME/agent-integrations/plugin-shared/lib/uri-guard.mjs" ] \
      && [ -f "$OV_HOME/agent-integrations/plugin-shared/lib/agent-hook-runtime.mjs" ] \
      && [ -f "$OV_HOME/agent-integrations/cursor/plugin.json" ] \
      && [ -f "$OV_HOME/agent-integrations/cursor/integration.json" ] \
      && [ -f "$HOME/.cursor/rules/openviking.mdc" ] \
      && [ -f "$HOME/.cursor/skills/openviking/SKILL.md" ]; then
  ...
  if contains_harness trae; then
    ...
    if grep -q 'scripts/hook.mjs' "$HOME/.trae/hooks.json" 2>/dev/null \
      ...
      && [ -f "$OV_HOME/agent-integrations/plugin-shared/lib/agent-hook-runtime.mjs" ]; then
  ...
  if [ -n "$MKT_DIR" ] && [ -f "$MKT_DIR/claude-code-plugin/scripts/marketplace.test.mjs" ] && [ -d "$MKT_DIR/../.git" ]; then
    node --test "$MKT_DIR/claude-code-plugin/scripts/marketplace.test.mjs" \
      "$MKT_DIR/codex-plugin/scripts/marketplace.test.mjs" || ok=0
  fi
