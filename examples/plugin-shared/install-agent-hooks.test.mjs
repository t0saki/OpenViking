...
const installer = join(checkout, "examples", "plugin-shared", "install.sh");
...
`);
    ...
    const sharedRoot = join(home, ".openviking", "agent-integrations", "plugin-shared");
...
test("combined hook-host install preserves unrelated hooks and is idempotent", () => {
  ...
  try {
    ...
    writeJson(traeHooks, { version: 1, hooks: { Stop: [
      ...
      { hooks: [{ type: "command", command: "OPENVIKING_HOOK_SOURCE=trae node /tmp/openviking/claude-code-plugin/scripts/trae-auto-capture.mjs" }] },
    ] } });
    ...
    const doctor = spawnSync(
      process.execPath,
      [join(home, ".openviking", "agent-integrations", "cursor", "scripts", "ov-plugin-doctor.mjs"), "cursor", "--offline", "--no-color"],
    ...
    assert.equal(existsSync(join(home, ".openviking", "agent-integrations", "plugin-shared")), false);
  } finally {
    rmSync(home, { recursive: true, force: true });
  }
});
...
test("uninstall with no installer runtime on disk removes what it can and fetches nothing", () => {
  ...
  try {
    ...
    rmSync(join(home, ".openviking", "agent-integrations", "plugin-shared"), {
      recursive: true,
      force: true,
    });
