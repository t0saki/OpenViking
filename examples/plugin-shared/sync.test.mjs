  ...
  for (const target of await resolveTargets()) {
    for (const file of target.files) {
      ...
      assert.equal(
        actual,
        expected,
        `${relative(ROOT, join(target.dir, file))} is out of sync; run node examples/plugin-shared/sync.mjs`,
      );
    }
  }
});
...
test("what each plugin ships equals the closure of what it imports", async () => {
  for (const target of await resolveTargets()) {
    ...
    assert.deepEqual(
      onDisk,
      target.files,
      `${relative(ROOT, target.dir)} does not hold the closure of its imports; run node examples/plugin-shared/sync.mjs`,
    );
  }
});
...
test("vendored skills are byte-identical to examples/skills", async () => {
  for (const { skill, dir } of SKILL_TARGETS) {
    ...
    assert.deepEqual(
      ...
      files,
      `${relative(ROOT, target)} has a different file set; run node examples/plugin-shared/sync.mjs`,
    );
    for (const file of files) {
      assert.equal(
        await readFile(join(target, file), "utf-8"),
        await readFile(join(SKILLS_DIR, skill, file), "utf-8"),
        `${relative(ROOT, join(target, file))} is out of sync; run node examples/plugin-shared/sync.mjs`,
      );
    }
  }
});
