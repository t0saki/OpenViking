...
test("the Claude Code adapter is the extractCaptureTurns its callers import", async () => {
  const cc = await import("../claude-code-plugin/scripts/cc-transcript.mjs")
  const vendored = await import("../claude-code-plugin/scripts/shared/capture-utils.mjs")
...
test("the Codex adapter is the extractCaptureTurns its callers import", async () => {
  const codex = await import("../codex-plugin/scripts/capture-utils.mjs")
  const vendored = await import("../codex-plugin/scripts/shared/capture-utils.mjs")
