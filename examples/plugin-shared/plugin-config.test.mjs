...
import { loadConfig as loadClaudeCode } from "../claude-code-plugin/scripts/config.mjs";
import { loadConfig as loadCodex } from "../codex-plugin/scripts/config.mjs";
import { loadConfig as loadOpencode } from "../opencode-plugin/lib/config.mjs";
import { resolveConfig as loadDsh } from "../dsh-plugin/config.mjs";
...
const LOADERS = {
  claude_code: {
    ...
    options: { manifestUrl: new URL("../claude-code-plugin/.claude-plugin/plugin.json", import.meta.url), logFile: "cc-hooks.log", rootKeyFallback: true },
    owns: ["configPath", "credentialPath"],
  },
  codex: {
    harness: "codex",
    load: (cwd) => loadCodex(cwd),
    options: { manifestUrl: new URL("../codex-plugin/.codex-plugin/plugin.json", import.meta.url), logFile: "codex-hooks.log" },
    owns: [],
  },
  opencode: {
    harness: "opencode",
  ...
  dsh: {
    ...
    options: { manifestUrl: new URL("../dsh-plugin/package.json", import.meta.url), deriveEffectivePeer: true },
    owns: ["peerId"],
  },
  pi: {
    harness: "pi",
