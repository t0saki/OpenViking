...
6. The extension reads `<archive_uri>/.overview.md` directly, once. It never substitutes an older session-context overview. A summary that is not there yet — phase 2 usually needs longer than a handler may wait — persists the pending archive and frozen boundary; every later `turn_end` and the next `before_agent_start` read that archive once, without committing again. When the commit's task has ended (`completed`, `failed`, `cancelled`, or no longer known to the server) and a last read still finds no summary — a failed summary, or Working Memory disabled on the server — the pending archive is dropped and its frozen token pressure spent, so the next archive waits for fresh pressure. A pending archive whose boundary left the active context is dropped without a read.
...
| Commit task ended without an overview | Drop the pending archive and spend its frozen token pressure; the boundary stays where it was |
