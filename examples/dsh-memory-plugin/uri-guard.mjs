...
import { evaluateUriGuard, evaluateUriNotice, isSkillUri } from "./shared/uri-guard.mjs";
...
const GUARDED_TOOLS = {
  ...
  edit: {
    ...
    example: uri =>
      isSkillUri(uri)
        ? `${mcp("add_skill")}(data="<the full edited SKILL.md text>")`
  ...
  write: {
    ...
    example: uri =>
      isSkillUri(uri)
        ? `${mcp("add_skill")}(data="<the full SKILL.md text>")`
