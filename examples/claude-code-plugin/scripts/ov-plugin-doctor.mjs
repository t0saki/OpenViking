...
function checkInstall(report, { cliOnPath }) {
  ...
  if (!installed.exists) {
    ...
    if (!entries.length) {
      ...
      if (installPath && resolvePath(installPath) !== PLUGIN_ROOT) {
        const registeredDoctor = join(installPath, "scripts", "ov-plugin-doctor.mjs");
  ...
  if (cliOnPath) {
    ...
    if (list.ok) {
      ...
      const others = rows.filter((r) => r?.id !== PLUGIN_ID && /openviking|claude-code-plugin/.test(String(r?.id)));
