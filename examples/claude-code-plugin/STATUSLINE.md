...
- **`$REPO`** = the OpenViking repo root. Default: `~/.openviking/openviking-repo` (override via `OPENVIKING_REPO_DIR` at install time). Verify with `ls "$REPO/examples/claude-code-plugin"` — if that path is wrong, find the real one with `jq -r '.statusLine.command' ~/.claude/settings.json` (the registered command points into the plugin) or `find ~ -path '*/claude-code-plugin/scripts/statusline.mjs' 2>/dev/null`.
- **`$PLUGIN`** = `$REPO/examples/claude-code-plugin`. The plugin's own root.
