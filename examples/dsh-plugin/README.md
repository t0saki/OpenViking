...
./examples/dsh-plugin`) only works when that checkout has its own
`node_modules`, because Node resolves the bundle's dsh peers from the source
tree's realpath rather than from the profile.
It also needs `node examples/plugin-shared/sync.mjs` run first: the
...
`node examples/plugin-shared/sync.mjs` — edit
`examples/plugin-shared/lib/` and `examples/skills/`, never the vendored
copies.
