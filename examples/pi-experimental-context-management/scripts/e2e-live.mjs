#!/usr/bin/env node
/**
 * Live e2e for the experimental OpenViking pi extension.
 *
 * This is intentionally manual: it drives a real pi binary, a real
 * OpenViking server, and a real OpenAI-compatible LLM relay.
 *
 * Skeleton only for now — it exercises sync + commit + recall. The
 * context-window gate (scripts/e2e-window.mjs) builds on it in a later phase.
 *
 * Required env:
 *   OPENVIKING_URL
 *   OPENVIKING_API_KEY
...
function runTurn(turn, prompt, extraArgs = []) {
  ...
  const res = spawnSync(PI_BIN, args, {
    ...
    env: {
      ...
      OV_DEBUG_LOG: join(outDir, "ov-pi.log"),
    },
    timeout: 300_000,
    encoding: "utf8",
  });
...
const logPath = join(outDir, "ov-pi.log");
if (existsSync(logPath)) {
  const log = readFileSync(logPath, "utf8");
  check(log.includes("commit"), "debug log records a commit");
  console.log(`\ne2e: ov-pi.log:\n${log.trim()}`);
} else {
  warn("no debug log written");
}
