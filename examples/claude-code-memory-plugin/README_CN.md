...
`<d>` 是任意标点分隔符（`/`、`|`、`#`、`:`），模式里用 `\` 转义它。flags 支持 `i`、`m`、`s`、`u`、`g`（`g` 表示替换全部；对 `d`/`k` 无意义会被忽略）。最多 32 条规则，单条模式最长 512 字符。
...
- **环境变量是逗号分隔的列表**，先按逗号切分再解析。模式里的字面逗号可以写成 `\x2c`，但它不是量词语法，所以 `{10,}` 这种带下界的量词只能写在 `ovcli.conf` 数组里；`s` 的替换串里的逗号则完全无法用环境变量表达。
- **顺序有意义，丢弃优先。** 规则自上而下执行，第一条命中的 `d`（或未命中的 `k`）就决定了结果。被替换成空串不算丢弃 —— query 空了只是长度不够，不会去检索空串。
- **过滤发生得很早**：召回侧在 `OPENVIKING_MIN_QUERY_LENGTH` 之前，捕获侧在内置的应答语／slash 命令／信息量启发式之前，所以一段被剥掉前缀后只剩 `好的` 的文本会按应答语丢弃。
- **捕获规则只管送出去的内容，管不到已经存下的。** 已经写进 OpenViking 的回合不会被改写，pending 队列里的内容用的是入队当时生效的规则。会话中途新增 `d`/`k` 规则还会让游标计数的回合列表变短，这会被当成 transcript 被改写、从最后一个用户回合重放 —— 和切换 `OPENVIKING_CAPTURE_ASSISTANT_TURNS` 是同一个现象。
- **坏规则只会被跳过，不会让 hook 挂掉。** `ov-memory-doctor` 会列出生效的规则，并对编译失败的那条给出确切的解析或 RegExp 报错。至于会灾难性回溯的模式，那是自己的脚 —— 表现是 debug 日志里的 `slow`，最坏情况是 hook 超时。
- `u` flag 会拒绝一些不加它时合法的转义（`\-`、`\_`、单独的 `{`、`[\w-.]`），除非模式需要 Unicode 属性转义，否则别开。
