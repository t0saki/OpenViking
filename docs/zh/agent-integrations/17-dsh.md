...
| 装到的是旧版本，或报包「不在 npm registry 中」 | pnpm 默认拒绝发布不满 24 小时的版本（`minimumReleaseAge`），并把 `latest` 解析到最新的合规版本。指定精确版本即可——`dsh plugin --profile web add @openviking/dsh-memory-plugin@<version>`，pnpm 会装上并把豁免写进 profile 的 `pnpm-workspace.yaml`。安装器已经会自动指定版本 |
