...
安装器会把扩展复制到 `~/.pi/agent/extensions/openviking`，用 `pi install` 完成注册，并配置 `~/.openviking/ovcli.conf`。
...
`OPENVIKING_PEER_ID`、`ovcli.conf` 或 `ov.conf` 中的显式 peer 优先于 `plugin` 段里写的 `peerId`；这两种显式 peer 又都优先于 workspace 派生值。只有启用 `workspacePeer` 且所有显式来源都未提供 peer 时，才会从工作区派生。
