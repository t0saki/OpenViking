...
`peerSource` 决定 workspace peer 的派生方式。默认的 `"git"` 取仓库归一化后的 `origin` URL（`git@github.com:volcengine/OpenViking.git` 得到 `github.com-volcengine-openviking`），依次回退到仓库根路径和当前工作目录，因此同一个仓库的每个 clone、worktree 和子目录共用同一个 peer。`"cwd"` 恢复此前的行为——把工作目录路径中的非字母数字字符全部替换成 `-`；`"none"`（等同于 `OPENVIKING_WORKSPACE_PEER=0`）完全不发送 peer；也可以用 `"team-{dir}"` 这样的模板，或按顺序尝试的模板列表自定义。
