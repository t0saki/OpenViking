...
为 [pi](https://github.com/earendil-works/pi) 提供跨会话长期记忆和 context takeover。召回、捕获和 takeover 基于 pi 原生扩展 API：每个 prompt 前自动召回、每个 turn 后自动捕获，并可由 OpenViking 在 pi 的 `context` hook 中用归档 overview 替换已 commit 的本地历史。pi 自身没有 MCP 客户端，扩展在 pi 进程内运行与 Claude Code、Codex、OpenCode 插件同款的 MCP proxy core——不起子进程、不引入额外依赖——把服务端的 MCP 工具注册成 pi 原生工具。
