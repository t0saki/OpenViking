...
| 宿主能注册原生工具，但没有 MCP 客户端 | 在进程内驱动共享 MCP proxy core，把服务端的 `tools/list` 注册成加 `openviking_` 前缀的宿主原生工具；不得自行维护工具目录 | [pi](https://github.com/volcengine/OpenViking/blob/main/examples/pi-coding-agent-extension/README.md) |
