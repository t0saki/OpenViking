# TRAE、TRAE CN 与 TraeCode CLI 2.0 集成
...
5. 对 TraeCode CLI 2.0，运行 `trae-cli plugin list` 确认 `openviking` 已启用，并在会话里输入 `/hooks`，确认 OpenViking 的条目已信任且处于开启状态。
...
将 `trae-cn` 替换为 `trae` 可管理 TRAE 集成。TraeCode CLI 2.0 请执行 `trae-cli plugin uninstall openviking@openviking`。安装器的 `--harness trae-cli --uninstall` 只用于移除旧安装中已弃用的独立 Hooks 集成。
...
| TraeCode CLI 2.0 插件已装，但召回和捕获都不发生 | 启动时的 hook 信任确认被跳过，或当时选了 *Continue without trusting*；插件更新动了 hook 后也会重新要求信任。`/hooks` 里信任并开启 OpenViking 的条目，`/plugins` 里确认 `openviking` 已启用——两个开关相互独立，都要是开着的。 |
...
| TraeCode CLI 2.0 未列出插件 | 运行 `trae-cli plugin list`；若没有 `openviking`，使用 `--harness trae-cli` 重跑安装器。 |
