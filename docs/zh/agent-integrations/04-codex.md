...
两个过滤器 knob 同样可以写在 `ovcli.conf` 里，值是 JSON 数组：`plugin.codex.recallQueryFilters` 只对这个 harness 生效，`plugin.recallQueryFilters` 对所有 harness 生效。建议优先用这种写法——环境变量是按逗号切分的列表，所以规则里带字面逗号（比如带下界的 `{10,}`）只能写进数组。
