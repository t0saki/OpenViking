...
| 工具 | 说明 | 主要参数 |
|------|------|----------|
| `find` | 无 session 上下文的快速语义检索。`context_type="skill"` 且不传 `target_uri` 时同时检索自己的 skill 和账户共享的 `viking://agent/skills` | `query`, `target_uri`(可选), `limit`, `min_score`, `level`(可选), `context_type`(可选) |
