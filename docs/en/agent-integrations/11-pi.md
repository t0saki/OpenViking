...
```bash
bash <(curl -fsSL https://raw.githubusercontent.com/volcengine/OpenViking/main/examples/plugin-shared/install.sh) --harness pi
```
...
| Load fails with a missing `shared/*.mjs` module (for example `shared/capture-utils.mjs`) | The copy was made without running `sync.mjs` first. Run `node examples/plugin-shared/sync.mjs` from the repository root and copy again, or re-run the installer |
