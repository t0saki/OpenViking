...
The installer copies the extension into `~/.pi/agent/extensions/openviking`, registers it with `pi install`, and configures `~/.openviking/ovcli.conf`.
...
An explicit peer from `OPENVIKING_PEER_ID`, `ovcli.conf`, or `ov.conf` takes precedence over a `peerId` set in the `plugin` section. Either explicit peer takes precedence over the workspace-derived peer, which is used only when `workspacePeer` is enabled and no explicit source provides a peer.
