...
1. **Vendoring (copying)**: The `sync.mjs` script distributes these modules to 7 targets, prefixing every file with `// GENERATED FROM ... DO NOT EDIT.`. Because of this added line, a vendored copy's line number will be exactly one line greater than the library source. There is no hand-kept file list: each target gets the transitive closure of the modules its own code imports. Claude Code, Codex and agent-plugins are installed by pointing a host at a directory in this repository, so their generated copies are committed and a push to main regenerates them. OpenCode, DSH and OpenClaw publish as npm packages and pi is tarred by the installer, so those four generate their copies at pack time and keep none in git.
