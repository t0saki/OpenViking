...
| kimicode | MCP passthrough | 15 | ✅ | ✅ | ✅ | ✅ | ✅ | ❌¹ | no type distinction² |
...
| kimicode | 7 hooks + MCP proxy | ✅ | ✅ | ❌ | ✅ (6000, on the first prompt) | ❌ | ✅ | ❌ |
...
- **Thin hook** (sharing `agent-hook-runtime`, meaning core behaviors are essentially identical, with differences limited to host events and thresholds): cursor, trae/trae-cn, zcode, and kimicode.
...
The `examples/memory-plugin-shared/lib/` directory contains 24 `.mjs` modules and serves as the single source of truth for all JS-based harnesses. These are consumed in two ways:

1. **Vendoring (copying)**: The `sync.mjs` script distributes these modules to 7 targets, prefixing every file with `// GENERATED FROM ... DO NOT EDIT.`. Because of this added line, a vendored copy's line number will be exactly one line greater than the library source (keep this in mind when cross-referencing line numbers). There is no hand-kept list of what each target gets: the set is the transitive closure of what that target's own code imports, so it follows the import graph rather than the harness. claude-code takes 24 modules, codex 23, opencode 21, dsh and pi 19 each, openclaw 2 (its own TypeScript covers the rest), and agent-plugins 5 — `credentials`, `debug-log`, `ov-http`, and the two `mcp-proxy-*` — because it has no hooks. When the copies are made follows how the plugin is delivered: claude-code, codex and agent-plugins are installed by pointing a host at a directory in this repository, so their copies are committed and a push to main regenerates them; opencode, dsh and openclaw publish as npm packages and pi is tarred by the installer, so those four build their copies at pack time and keep none in git.
2. **Direct import via relative path (no copying)**: cursor, trae, trae-cn, zcode and kimicode directly `import "../../memory-plugin-shared/lib/..."`. To ensure this works, the installer copies the package alongside the 23 shared modules those hooks transitively import into `~/.openviking/agent-integrations/{<client>,memory-plugin-shared}/`, preserving the relative folder layout. That installed set is closed under imports, down to the workspace configuration layer those hook runtimes resolve their settings through. At runtime, these harnesses share this directory, meaning reinstalling any one of them will overwrite the shared directory wholesale.
...
| `capture-utils.mjs` | Handles message normalization, injection back-flow guarding, and capture filtering | cc / codex / opencode / dsh / pi / zcode / cursor / trae×2 / kimicode |
...
| `async-writer.mjs` | Detaches the write path (drains stdin → spawn → approve → write → unref). Falls back to synchronous writing if spawn fails | cc / codex / zcode / kimicode |
...
| `uri-guard.mjs` | Intercepts cases where `viking://` is incorrectly treated as a local path | Used in each harness's `PreToolUse` or `tool.execute.before`-style hooks |
...
| kimicode | Config-driven (a comment-delimited block in `~/.kimi-code/config.toml`, plus `~/.kimi-code/mcp.json`) | One-line `--harness kimicode` | `kc-<session_id>` | env + ovcli.conf `plugin.kimicode` | ❌ |
...
| pi | Native pi extension (loaded from a directory, with TypeScript transpiled on the fly via jiti) | One-line `--harness pi` (including `pi install` registration) | `pi-<piSessionId>` | env + ovcli.conf `plugin.pi` (credential fields are resolved via the shared credential chain) | ✅ |
...
- **Uninstallation:** The `--uninstall` flag covers `cursor`, `trae`, `trae-cn`, `zcode`, and `kimicode`, and also cleans up any legacy trae-cli hook config. The Codex-format and host-managed plugins (`claude`, `codex`, `trae-cli`, `opencode`, and `pi`) are removed via their respective host's plugin management systems.
...
4. **apiKey**: Resolves via `BEARER_TOKEN` → `API_KEY` → `ovcli` `api_key` → `ovcli` `plugin.<harness>.apiKey` → `ovcli` `plugin.apiKey` → `ov.conf` `<harness>.apiKey` → `server.root_api_key`. The `<harness>` block is the one named after the calling harness (snake_case, so `trae_cn`; `claude-code` and `claude_code` reach the same block), and `account` / `user` / `peerId` read that same block at the same position in their own chains.
5. **mcpUrl**: Resolves via `OPENVIKING_MCP_URL` (when outside CLI mode) → `${baseUrl}/mcp`.
6. **Common request headers**: `Authorization: Bearer` + `X-OpenViking-Account/User` (trusted mode only) + `X-OpenViking-Actor-Peer` + `User-Agent: openviking-memory-<harness>/<version>`. An `api_key` server reads the identity out of the key and ignores those two headers, so they are withheld there rather than named to every proxy on the path. The mode resolves via `ovcli` `plugin.<harness>.authMode` → `plugin.authMode` → `ov.conf` `<harness>.authMode` → `ov.conf` `server.auth_mode` → trusted whenever an account or user resolved at all.
...
- `OPENVIKING_COMMIT_TURN_THRESHOLD`: cursor only (`trae`, `trae-cn`, `zcode` and `kimicode` commit on every captured turn and ignore this threshold).
...
| kimicode | `UserPromptSubmit` | prior injection blocks stripped | ✅ `kc-` | A | plain-text stdout (a JSON envelope would be shown to the user as context) | ❌ |
...
- **Who injects, when, and with what budget**:
  ...
  - **kimicode**: On the first `UserPromptSubmit` of the session (6000 budget) — its `SessionStart` observes only and cannot inject.
...
- **Actual timeout values**:
  ...
  - **cursor / trae×2 / zcode / kimicode**: 15s (against a 20s host hook budget).
...
- **trae / zcode / kimicode**: Utilize their own cleaning functions (zcode's strips three distinct types of injection blocks; kimicode's runs over the prompt and over the wire transcript alike).
...
| kimicode | Every capture carrying content commits (no threshold), keep 0; the wire cursor advances conservatively, so missed turns are caught up on the next capture in the same session | SessionEnd: unconditional; Interrupt: captures inline, without detaching | PreCompact: the same capture and commit |
...
| kimicode | **C** (`SessionEnd` captures and commits) | **C\*** | **—** | **—** | **—** | C\* precondition: the interrupt reaches the running turn, where `Interrupt` fires instead of `Stop` and captures inline. Every capture with content has already committed, and whatever a path skips is recovered by the wire cursor at the next capture in the same session |
...
1. Six integrations commit on a normal exit: `claude-code`, `opencode` (≥1.15.11), `dsh`, `pi` (takeover off), `kimicode`, and `hermes`. The rest rely on the recovery mechanisms detailed in the "recovery path" column.
...
3. `trae×2`, `zcode` and `kimicode`, which commit on every turn, offer the simplest shutdown semantics (the maximum data left to archive equals the last round that never reached Stop), at the cost of a full archive and memory extraction on every Stop (keep 0).
...
| cursor / trae×2 / zcode / kimicode / pi / openclaw | No specific subagent handling (anything generating its own session ID becomes its own session; otherwise, it mixes into the main session. `openclaw` can mask this behavior using `bypassSessionPatterns`) |
...
| cursor / trae×2 / zcode / kimicode | No takeover | Cursor and kimicode commit unconditionally on their compaction event (Trae×2/Zcode have none upstream). | — |
...
| cursor/trae×2/zcode/kimicode | Fetch errors are swallowed as `status:0`, and catch returns an empty injection; silently skips if the lock isn't acquired within 5s | context-face 6h (no negative cache for unreachable servers; every turn waits the full 15s) | None | No |
...
- **Form**: A Claude Code plugin (marketplace) featuring a four-in-one architecture: 9 hooks, an MCP proxy (passing through 15 tools), a slash command, a statusline, and 1 experience skill. Version 0.6.0.
...
- **Form**: A Codex plugin (marketplace). Features 5 hooks (`SessionStart` 70s / `UserPromptSubmit` 130s / `Stop` 30s / `SessionEnd` 3s / `PreCompact` 60s), an MCP proxy, and 1 experience skill. Version 0.10.0.
...
- **Form**: Config-driven (a comment-delimited block in `~/.kimi-code/config.toml`, MCP in `~/.kimi-code/mcp.json`) utilizing an MCP proxy. It features 7 hooks: SessionStart(30s) / UserPromptSubmit(20s) / PreToolUse:Read\|Glob\|Grep(5s) / Stop(30s) / PreCompact(30s) / SessionEnd(30s) / Interrupt(10s). It vendors nothing: the installer assembles its shared runtime the way it does for cursor, trae and zcode. Version 0.4.0.
- **Capability highlights**: The only host in this family with `SessionEnd`, `PreCompact` and `Interrupt`, so a shutdown does not depend on the last Stop. `agents/main/wire.jsonl`, found through `session_index.jsonl`, is the source of truth for increments, deduplicated on the host's own `turnId`. `KIMI_CODE_HOME` moves the whole configuration root. It also ships a native `kimi.plugin.json`, so `/plugins install <path>` works against an installed copy.
- **Behavior notes**: The injection is plain stdout text, not JSON — an envelope would be appended to the conversation for the user to read — and only `UserPromptSubmit` may inject at all, which is why the profile rides the first prompt instead of the session start. Hook stdin is snake_case (`session_id`, `hook_event_name`, `cwd`, `tool_name`, `tool_input`). Identical `command` strings are de-duplicated by the host and run once. Sessions are `kc-`; every capture that carries content commits (keep 0).
- **Configuration**: Configured via environment variables, `ovcli.conf` (`plugin.kimicode`), and the workspace files; `OPENVIKING_WRITE_PATH_ASYNC` takes effect for kimicode.
- **Dimension index**: tool surface [§2.1](#_2-1-server-side-mcp-tool-surface) | recall [§3.2](#_3-2-automatic-recall-and-injection) | commit [§3.3.2](#_3-3-2-regular-commit-triggers)/[§3.3.3](#_3-3-3-shutdown-method-×-harness-outcome-matrix).

## opencode

- **Integration docs**: [OpenCode Plugin](./10-opencode.md)
- **Form**: Provided as the npm plugin `@openviking/opencode-plugin`, featuring a config hook that injects the MCP entry itself (tools carry the `openviking_` prefix). It exposes 7 plugin hooks: config / event / tool.execute.before / experimental.chat.system.transform / chat.message / experimental.session.compacting / dispose. Version 0.4.0.
...
- **Form**: Operates as a native pi extension (loaded from a directory and dynamically transpiled from TS by jiti). It registers 7 native `viking_*` tools and uses direct REST communication (since pi lacks MCP support). It features 8 events + a `/viking` command. Version 0.4.0.
