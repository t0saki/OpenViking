...
- **MCP-based harnesses (claude-code, codex/trae-cli, cursor, trae/trae-cn, zcode, opencode, dsh, pi) share an identical active tool surface comprising 15 tools.** The server centrally defines these tools. The plugins read `~/.openviking/ovcli.conf` and connect to the server-defined MCP tools. pi uses the official MCP client inside its extension and registers the discovered tools as native pi tools.
...
| pi | MCP mirror (official client; extension adds an `openviking_` prefix) | 15, i.e. whatever `tools/list` returns (registration has preconditions³) | ✅ | ✅ | ✅ | ✅ | ✅ | ❌¹ | no type distinction² |
