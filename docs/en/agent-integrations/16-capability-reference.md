...
- **Form**: Config-driven (merged into `~/.zcode/cli/config.json`, forcing `hooks.enabled=true`) utilizing an MCP proxy. It features 4 hooks: SessionStart(30s) / UserPromptSubmit(20s) / PreToolUse:Read\|Glob\|Grep(5s) / Stop(30s). It vendors 19 shared modules, and is the only harness that vendors the `agent-hook-runtime` pair instead of importing it by relative path. Version 0.1.1.
