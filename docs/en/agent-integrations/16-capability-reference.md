...
| `capture-utils.mjs` | Handles message normalization, injection back-flow guarding, and capture filtering | cc / codex / opencode / dsh / pi / zcode / cursor / trae×2 |
...
| **B. openclaw** (its own `config.ts`) | openclaw | `OPENVIKING_BASE_URL` → `OPENVIKING_URL` | `OPENVIKING_API_KEY` (supports SecretRef env/file) | `OPENVIKING_ACCOUNT_ID` / `OPENVIKING_USER_ID` (note the `_ID` suffix here) | Sends both `X-API-Key` and `Authorization: Bearer`. The server reads `X-API-Key` first, so gateways routing on it keep working. |
...
Family B ends in the same chain: `openclaw.json` and openclaw's own `OPENVIKING_*` spellings answer first, and a `baseUrl` / `apiKey` / `accountId` / `userId` still empty afterwards resolves through `resolveOpenVikingCredentials(env, "openclaw")` — `ovcli.conf`, then the `openclaw` block of `ov.conf`, then `server.root_api_key`, with `OPENVIKING_CREDENTIAL_SOURCE` pinning that shared layer as it does for Family A. What openclaw does not read is the ovcli.conf `plugin` section, which needs the knob schema.
...
- **Form**: Represents the only full ContextEngine takeover (`ownsCompaction:true`). Features 15 native tools (14 enabled by default) + 5 slash commands + 4 hooks + Gateway HTTP routes + feature-gate RPC. It operates entirely remotely. Version 2026.7.0.
...
- **Config**: Configured via `plugins.entries.openviking.config` in `openclaw.json` alongside a few environment variables, with any field still empty falling back to the shared credential chain. Sends both the `X-API-Key` and `Authorization: Bearer` auth headers (see [§3.1.3](#_3-1-3-credential-systems) for both). The commit threshold is controlled by `commitTokenThresholdRatio` (default 0.5), and the recall character budget is 4000.
