...
1. **Vendoring (copying)**: The `sync.mjs` script distributes these modules to 7 targets, prefixing every file with `// GENERATED FROM ... DO NOT EDIT.`. Because of this added line, a vendored copy's line number will be exactly one line greater than the library source (keep this in mind when cross-referencing line numbers). There is no hand-kept list of what each target gets: the set is the transitive closure of what that target's own code imports, so it follows the import graph rather than the harness. claude-code and codex take 21 modules each, opencode 20, dsh and pi 18 each, openclaw 2 (its own TypeScript covers the rest), and agent-plugins 4 — `credentials`, `debug-log`, and the two `mcp-proxy-*` — because it has no hooks. When the copies are made follows how the plugin is delivered: claude-code, codex and agent-plugins are installed by pointing a host at a directory in this repository, so their copies are committed and a push to main regenerates them; opencode, dsh and openclaw publish as npm packages and pi is tarred by the installer, so those four build their copies at pack time and keep none in git.
2. **Direct import via relative path (no copying)**: cursor, trae, trae-cn and zcode directly `import "../../memory-plugin-shared/lib/..."`. To ensure this works, the installer copies the package alongside the 22 shared modules those hooks transitively import into `~/.openviking/agent-integrations/{<client>,memory-plugin-shared}/`, preserving the relative folder layout. That installed set is closed under imports, down to the workspace configuration layer those hook runtimes resolve their settings through. At runtime, these harnesses share this directory, meaning reinstalling any one of them will overwrite the shared directory wholesale.
...
| `capture-utils.mjs` | Handles message normalization, injection back-flow guarding, and capture filtering | codex / opencode / dsh / pi / zcode / cursor / trae×2 |
...
4. **apiKey**: Resolves via `BEARER_TOKEN` → `API_KEY` → `ovcli` `api_key` → `ov.conf` `<harness>.apiKey` → `server.root_api_key`. The `<harness>` block is the one named after the calling harness (snake_case, so `trae_cn`; `claude-code` and `claude_code` reach the same block), and `account` / `user` / `peerId` read that same block at the same position in their own chains.
5. **mcpUrl**: Resolves via `OPENVIKING_MCP_URL` (when outside CLI mode) → `${baseUrl}/mcp`.
6. **Common request headers**: `Authorization: Bearer` + `X-OpenViking-Account/User` (trusted mode only) + `X-OpenViking-Actor-Peer` + `User-Agent: openviking-memory-<harness>/<version>`. An `api_key` server reads the identity out of the key and ignores those two headers, so they are withheld there rather than named to every proxy on the path. The mode resolves via `ovcli` `plugin.<harness>.authMode` → `plugin.authMode` → `ov.conf` `<harness>.authMode` (claude-code, codex and dsh only — the three loaders that read that section as a settings layer) → `ov.conf` `server.auth_mode` → trusted whenever an account or user resolved at all.
...
| ov.conf harness sections (`<harness>.*`, legacy) | Every harness, for the section named after it | The credential fields (`apiKey`, `accountId`, `userId`, `peerId`) resolve from the calling harness's own section; the tuning knobs do so on claude-code, codex and dsh. Either spelling of the name reaches the same block. |
