...
| opencode | dual-stack v1/v2 plugin hooks + MCP proxy | ✅ | ✅ | ❌ | ✅ (10000) + `<available-skills>` (1200) + repo list into the system prompt | ❌ (commit after compaction only) | ✅ | ❌ |
...
| opencode | v1: every `chat.message`; v2: every `prompt` | v1 concatenates non-synthetic text parts; v2 reads prompt text directly | ✅ `oc-` | A (timeoutMs=30000) | v1 prepends a synthetic part; v2 stores message metadata and injects before that user message in the context hook | ❌ |
...
| opencode | v1 `session.idle` / v2 execution completion: after flush, `pending_tokens ≥ 20000` triggers commit, keep 10 | `session.deleted` / `session.error`: forced commit; v1 dispose / v2 cleanup: forced commit | v1 fires before compacting and after compacted; v2 fires once after `session.compaction.ended` |
...
| opencode | **C\*** (v1 ≥1.15.11 `dispose` or v2 cleanup calls `flushAll({commit:true})`) | **C\*** | **C\*** | **C\*** | **—** | C\* precondition: host cleanup time is finite, so slow requests or serial commits across several sessions may be cut off; v2 cleanup runs after 60 minutes of inactivity, service shutdown, and local-plugin hot reload |
...
| opencode | `oc-<parent>__subagent-<child>` hangs under the parent namespace; session-start injection skips subagents (recall does not); v2 maps `parentID` from `session.created`, then reads context when execution ends |
...
| opencode | No takeover | v1 flushes and commits before compaction; v2 captures the transcript before compaction | v1 commits again on `session.compacted`; v2 commits once on `session.compaction.ended` |
...
| opencode | Recall, context, after-tool, event-loop, and cleanup failures are caught and logged; intentional URI-guard denials still reach the model | context-face 6h only; `/health` is uncached | No synchronous retry; the MCP proxy retries once each for 401/403 and 400/404 | No (URI-guard denial is intentional) |
...
| trae/trae-cn | ❌ | ❌ | None | ❌ | — |
| zcode | ❌ | ❌ | None | ❌ | — |
| opencode | ❌ | ❌ | None (Deliberately omitted) | ✅ | v1 has toasts; v2 logs only |
...
- **Form**: The npm plugin `@openviking/opencode-plugin` 0.5.0 supports OpenCode v1 ≥1.15.7 and v2 ≥2.0.15 from one entrypoint. v1 retains its existing hooks; v2 uses `setup(ctx)`, prompt/context/tool hooks, and structured lifecycle events. MCP tools keep the `openviking_*` prefix, with v2 explicitly setting `codemode:false`.
- **Capability highlights**: On v2, the prompt hook recalls once and stores context in user-message metadata; later model steps inject the same stored content, avoiding repeated requests and prompt-cache churn. At execution completion, `session.context()` supplies complete user, assistant, and tool messages for capture. The repository list remains in the system prompt ([§3.2.3](#_3-2-3-profile-opening-injection)).
- **Behavior notes**: `commitTokenThreshold` is 20000 and commit timeout is 30000ms. v2 checks the threshold after succeeded/interrupted executions and force-commits on failed/deleted executions, completed compaction, and cleanup; a user interruption retains state. v2 cleanup runs after 60 minutes of inactivity, service shutdown, or local-plugin hot reload. v2 has no toast API and logs notices instead. Opening injection is attempted once per session ([§3.2.3](#_3-2-3-profile-opening-injection)).
