...
- **MCP-based harnesses (claude-code, codex/trae-cli, cursor, trae/trae-cn, zcode, opencode, dsh, pi) share an identical active tool surface comprising 15 tools.** The server centrally defines these tools. The plugin reads `~/.openviking/ovcli.conf` via its proxy and establishes a connection to the server-defined MCP tools. They differ only in the prefix each host puts on a tool name and in how the proxy runs: pi has no MCP client of its own, so its extension drives the same proxy core in process, with no subprocess and no stdio.
...
| pi | MCP mirror (the extension bridges `/mcp` in process and adds an `openviking_` prefix) | 15, i.e. whatever `tools/list` returns (registration has preconditions³) | ✅ | ✅ | ✅ | ✅ | ✅ | ❌¹ | no type distinction² |
...
| `mcp-proxy-core.mjs` | stdio ↔ streamable-HTTP MCP proxy core | All MCP-based integrations + agent-plugins; pi drives the same core in process instead of over stdio |
...
- **Form**: Operates as a native pi extension (loaded from a directory and dynamically transpiled from TS by jiti). pi has no MCP client, so the extension drives the shared stdio↔HTTP proxy core in process — no subprocess, no stdio, no MCP client library — and republishes every descriptor from the server's `tools/list` as a pi tool named `openviking_<tool>`: 15 today ([§2.1](#_2-1-server-side-mcp-tool-surface)). Nothing in the extension enumerates tools, so a server that gains or drops one changes pi's surface at the next session with no plugin release. Recall, session sync, profile injection and takeover still go over REST. It features 9 events + a `/viking` command; `tool_call` blocks read/grep/find/ls/write/edit calls whose path is a `viking://` URI, and `tool_result` appends a notice to the result of a `bash` command that carries one. Version 0.4.0.
