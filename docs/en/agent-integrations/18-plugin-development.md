...
| The host registers native tools but has no MCP client | Drive the shared MCP proxy core in process and register the server's `tools/list` as native host tools under an `openviking_` prefix; do not hand-maintain a tool catalogue | [pi](https://github.com/volcengine/OpenViking/blob/main/examples/pi-coding-agent-extension/README.md) |
