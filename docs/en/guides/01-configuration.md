...
| `recall_intent_timeout_s` | float | Timeout for opt-in session-aware recall query expansion. On timeout recall silently falls back to the original query. Tune to your model endpoint latency. | `10.0` |
| `recall_rewrite_timeout_s` | float | Timeout for opt-in server-side recall digest rewriting. On timeout the unrewritten rendered context is returned. Sized for a full-scale VLM emitting ~300-500 tokens; a lightweight `query_planner` finishes much faster. | `20.0` |
