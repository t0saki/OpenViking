...
- Under a skills root (`viking://user/<user_id>/skills` or `viking://agent/skills`), writing `<root>/<name>/SKILL.md` installs or updates that skill the same way `POST /api/v1/skills` does: the frontmatter becomes the L0 abstract, the body becomes the L1 overview, and the skill's other files are kept. The frontmatter `name` must equal `<name>`, and `append` is not supported for `SKILL.md`. Other files under a skills root, except `.abstract.md`/`.overview.md` (which follow the sidecar rule above), are stored as-is, without semantic refresh or vectors, and ignore `tags`.
