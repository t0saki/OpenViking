...
| `{harness}` | The name of the agent running (`claude-code`, `codex`, `dsh`, `opencode`, `pi`, `cursor`, `trae`, `trae-cn`, `zcode`, `kimicode`) | Never — but the MCP proxy takes no part in derivation, so a read path that goes only through it cannot resolve it |
