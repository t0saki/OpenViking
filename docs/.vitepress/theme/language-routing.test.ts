...
for (const [pathname, base, locale] of [
  ['/', '/', 'zh'], ['/index.html', '/', 'zh'], ['/en/', '/', 'en'],
  ['/zh', '/', 'zh'], ['/en/index.html', '/', 'en'],
  ['/guide/', '/guide/', 'zh'], ['/guide', '/guide/', 'zh'],
  ['/guide/en/', '/guide/', 'en'], ['/guide/zh/index.html', '/guide/', 'zh'],
]) {
  test(`entry ${pathname} respects path language and base ${base}`, () => {
    ...
    assert.equal(next, `https://docs.openviking.ai${base}${locale}/getting-started/01-introduction?lang=zh&source=share#title`)
    assert.equal(docsLanguageEntry(next!, base, policy), null)
    assert.deepEqual(fixture.writes, [])
  })
}

test('root query precedes manual preference; localized documents never redirect', () => {
  const fixture = testBrowser({ cookie: 'openviking-language-preference=zh', url: 'https://docs.openviking.net/en/?lang=en' })
  const policy = createLanguagePreference(fixture.browser)
  assert.equal(docsLanguageEntry('https://docs.openviking.net/?lang=en', '/', policy), 'https://docs.openviking.net/en/getting-started/01-introduction?lang=en')
  for (const path of ['/en/guides/03-deployment', '/zh/guides/03-deployment', '/outside/', '/guidebook/']) {
  ...
  assert.equal(docsLanguageEntry('https://docs.openviking.net/', '/', policy), 'https://docs.openviking.net/zh/getting-started/01-introduction')
