...
- **Plugin npm Release** (`plugin-npm-release.yml`): runs automatically on pushes to `main` that touch `examples/dsh-plugin/**`, `examples/opencode-plugin/**`, or the shared plugin library `examples/plugin-shared/lib/**`, or manually. Publishes `@openviking/dsh-plugin` and `@openviking/opencode-plugin` to npm, one matrix job per package, and skips versions that already exist.
